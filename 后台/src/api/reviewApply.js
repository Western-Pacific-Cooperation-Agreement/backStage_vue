// 当前模块为**{{审核申请活动}}**请求api，API进行统一管理，即对请求接口统一管理
import request from '@/utils/axios' //api可以理解为普通axios的第二次封装
                                    //=>第一次主要是配置信息，第二次业务逻辑

var qs = require('qs')              //引入qs，会方便我们数据的转换操作

const BaseURL = '/reviewApply'//基础首页路径 =>对应后端请求的@RequestMapping("/act")
...
export const getActList = () => {
    console.log("http://localhost:18888"+BaseURL+get+'/actList')
  return request({
    url: BaseURL+get+'/actList',   // =>   http://localhost:18888/reviewApply/get/actList
    methods: 'get'
  })
}
